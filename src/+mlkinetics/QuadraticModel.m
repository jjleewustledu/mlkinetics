classdef (Abstract) QuadraticModel < handle & mlkinetics.Model
    ...
    methods
        ...
        function obs = obsFromTac(this, varargin)
            ...
            addParameter(ip, 'times_sampled', this.times_sampled, @isnumeric)
            addParameter(ip, 't0', this.t0, @isscalar)
            addParameter(ip, 'tF', this.tF, @isscalar)
            parse(ip, varargin{:})
            ipr = ip.Results;            
            assert(size(ipr.tac,4) == length(ipr.times_sampled))

            if size(ipr.tac,4) < 10
                times_more = linspace(ipr.times_sampled(1), ipr.times_sampled(end)); % N = 100
                ipr.tac = this.interpolate_img(ipr.tac, ipr.times_sampled, times_more);
            else
                times_more = ipr.times_sampled;
            end            
            window = ipr.t0 <= times_more & times_more <= ipr.tF;            
            obs = trapz(times_more(window), ipr.tac(:,:,:,window), 4);
        end
    end
    ...
    methods (Access = protected)
        function img = interpolate_img(~, img, times_sampled, times_more)
            sz = size(img);
            %tic
            [x,y,z,t] = ndgrid(1:sz(1), 1:sz(2), 1:sz(3), times_sampled);
            [xq,yq,zq,tq] = ndgrid(1:sz(1), 1:sz(2), 1:sz(3), times_more);
