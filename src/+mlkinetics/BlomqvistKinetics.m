classdef BlomqvistKinetics 
	...
	methods 
        function img  = buildCmrglcMap(this) 
            ...
            img = img * 60 * 100 * this.glc_ / this.LC; % 100*v1 -> CBV; 1/s -> 1/min
        ...
        function vox  = buildCmrglcVoxel(this, xvec)
            ...
            A  = [this.Acol1(this.Cp_,t,1) this.Acol2(this.Cp_,t,1) this.Acol3(qT,t)];
            
            opts = optimset('TolX', 1.e-4);
            lb_ = ([1e-4 1e-8 1e-4]/100)';
            ub_ = ([1e-2 1e-4 1e-2]*100)';
            kappa = lsqlin(A, qT', [], [], [], [], lb_, ub_, [], opts); 
            % kappa = lsqnonneg(A, qT', opts);            

            vox = kappa(2)/kappa(3); % K1 k3 / (k2 + k3) = v1 k1 k3 / (k2 + k3)
        end
        function this = buildCmrglcNonlinear(this, qt, qT, ks0) %#ok<INUSD>
            %% BUILDCMRGLCNONLINEAR calls fitnlm.
